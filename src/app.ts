...
import { subscribe, unsubscribe, unsubscribeFromAll } from "./db/subscription.js";
import { isSqlite3Error } from "./db/index.js";
...
app.post("/unsubscribe", async (req, res) => {
	try {
		...
		else res.status(422).json({ error: "Already unsubscribed!" });
	} catch (e) {
		if (e instanceof ZodError) {
			handleZodError(e, res);
		} else {
